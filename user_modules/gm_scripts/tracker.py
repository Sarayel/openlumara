...
# Display-pill colours for conditions across supported systems.
# Keys are matched verbatim against the condition names added via
# `tracker.py condition add <name> <condition>`. Unrecognised conditions
# fall back to a neutral grey pill in the display.
#
# The dict is system-agnostic and grows as new system modules ship —
# duplicates between systems (e.g. "stunned" in D&D and BRP) share a row,
# and system-specific conditions (e.g. "frenzied" in Vampire) coexist
# without conflict. See `SYSTEM-PORTING.md` for guidance when porting a
# new system module.
CONDITION_COLOURS = {
    # D&D 5e
    "unconscious":    "danger",
    ...
    # Vampire: The Masquerade (V5 + V20)
    # — Critical (red): taken out, Beast-controlled, or otherwise incapacitated
    "torpid":              "danger",   # vampire torpor — supernatural sleep
    "frenzied":            "danger",   # Beast in control (Hunger or Rage)
    "rotschreck":          "danger",   # fear frenzy — must flee fire/sunlight
    "staked":              "danger",   # heart pierced; paralyzed but aware
    "stasis":              "danger",   # suspended animation (V5 rare)
    # — Warning (amber): compromised, controlled, or under external pressure
    "dominated":              "warn",  # under another vampire's command
    "mesmerized":             "warn",  # Dominate-induced trance
    "blood-bound":            "warn",  # three-step blood bond (regnant loyalty)
    "blood-addicted":         "warn",  # vitae addiction (V20)
    "addicted":               "warn",  # vitae addiction (V5 alias)
    "vitae-taint":            "warn",  # carries another vampire's blood
    "masquerade-violation":   "warn",  # V5: Masquerade cracked
    "masquerade-risked":      "warn",  # V20: suspicion raised
    "clan-compulsion":        "warn",  # V5: active clan Compulsion
    "clan-weakness-active":   "warn",  # V20: active clan bane
    "impaired":               "warn",  # V5: half-Health -2 Physical dice
    "wound-penalty":          "warn",  # V20: cumulative dice penalty from injuries
    "path-deviation":         "warn",  # V20: acted against Path of Enlightenment
    # — Info (blue): passive or environmental flags
    "touched-by-blood":       "info",  # V5: Surfeit active
    "resonant":               "info",  # V5: carries Resonance
    "ghoul-bound":            "info",  # ghoul bound to a domitor
    "daylight-penalty":       "info",  # V5: daytime -2 to -5 dice
    "daytime-active":         "info",  # V20: daytime activity (sunlight risk)
    "possessed":              "info",  # supernatural entity riding the body
    # — Buff (green): actively protected or enhanced
    "masquerade-protected":   "buff",  # V5: passing as mortal
    "obfuscated":             "buff",  # Obfuscate active
    "auspex-protected":       "buff",  # V20: scanning with heightened senses
    "fortitude-active":       "buff",  # V20: aggravated soak available

    # Basic Roleplaying (BRP) — distinct conditions documented in systems/brp/system.md
    "bleeding":        "danger",
    "impaled":         "danger",
    "dying":           "danger",
    "major wound":     "warn",
