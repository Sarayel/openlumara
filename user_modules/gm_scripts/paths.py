...
def python_executable() -> str:
    """Return the Python interpreter to use for spawning subprocess scripts.

    Why this exists: scripts like scene_loader.py and debug_scene.py call
    back into other scripts via `subprocess.run(["python3", ...])`. On
    Windows, `python3` is not on PATH by default — the OS ships a stub
    that opens the Microsoft Store and prints "Python was not found"
    (exit 9009). Using `sys.executable` (the interpreter that's actually
    running the calling script) sidesteps the problem entirely.

    Prefer `sys.executable` when it's a real path (always true except in
    some embedded-interpreter edge cases). Fall back to `python3`/`python`
    on PATH. Last resort: the literal string "python3" so the error
